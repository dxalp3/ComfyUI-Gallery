...
export function HydrusToolbar({ onBrowseHydrus }: { onBrowseHydrus: () => void }) {
    ...
    return <div style={{ padding: '8px 4px 12px' }}>
        <Space wrap size={[8, 8]}>
            <Button icon={<DatabaseOutlined />} onClick={onBrowseHydrus}>Browse Hydrus</Button>
            ...
            {selectedImages.length > 0 && <>
                <Button disabled={!selectedImageUrls.length || busy} loading={busy} onClick={async () => {
                    setBusy(true); setNotice(undefined);
                    try { setNotice({ text: await appendLocalImages(selectedImageUrls), error: false }); }
                    catch (error) { setNotice({ text: error instanceof Error ? error.message : String(error), error: true }); }
                    finally { setBusy(false); }
                }}>Append to Image Source ({selectedImageUrls.length})</Button>
