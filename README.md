...
and selection avoid repeated work, and session caching reduces repeated Hydrus
status requests. See [validation results and limits](VALIDATION.md).

**[Installation, Hydrus setup, and usage → HYDRUS.md](HYDRUS.md)**

Maintained in [dxalp3/ComfyUI-Gallery](https://github.com/dxalp3/ComfyUI-Gallery),
based on [PanicTitan/ComfyUI-Gallery](https://github.com/PanicTitan/ComfyUI-Gallery).
The original gallery features are described below.
...
Stop ComfyUI, then run this command from `ComfyUI/custom_nodes`:

```bash
git clone https://github.com/dxalp3/ComfyUI-Gallery.git
```

Already using an archive or the original Gallery? Back up and move the existing
Gallery folder outside `custom_nodes` before cloning, then restore its settings
and Hydrus history into the new clone. Keep only one active Gallery installation.
See the [migration steps](HYDRUS.md#move-from-an-archive-or-the-original-gallery).
This fork contains the Hydrus integration; searching for the original Gallery in
ComfyUI Manager may install the upstream edition instead.

**2. Install Python Dependencies:**

Use ComfyUI's Python environment. From a standard ComfyUI checkout:

```bash
python -m pip install -r custom_nodes/ComfyUI-Gallery/requirements.txt
```

For Windows portable, use its embedded Python as described in [HYDRUS.md](HYDRUS.md#install-this-customized-version).
The compiled frontend is included, so Node.js is not required to install or update.
Start ComfyUI and hard-refresh its browser page.

**Updates:** Stop ComfyUI, back up your settings and Hydrus history, then run
`git pull --ff-only` inside `custom_nodes/ComfyUI-Gallery`. Reinstall Python
dependencies if `requirements.txt` changed, restart ComfyUI, and hard-refresh.
Settings and history are ignored by Git and are preserved. See the
[update guide](HYDRUS.md#update-a-git-installation) if you have local code changes.
